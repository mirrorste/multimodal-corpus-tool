...
| 阶段 | 计划时间 | 状态 | 完成度 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| **第一阶段**：基础框架搭建，视频下载功能 | 第 1-4 周 | 🔄 进行中 | 45% | 框架骨架已完成，核心业务逻辑待实现 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| FR-001 | YouTube 视频下载 | 高 | ⚠️ 部分完成 | 30% | API接口已定义，无实际下载逻辑 |
| FR-002 | Bilibili 视频下载 | 高 | ⚠️ 部分完成 | 30% | API接口已定义，无实际下载逻辑 |
| FR-003 | Vimeo 视频下载 | 中 | ⚠️ 部分完成 | 20% | API接口已定义，无实际下载逻辑 |
| FR-004 | 本地视频文件导入 | 高 | ⚠️ 部分完成 | 20% | 平台选项已包含，无文件上传逻辑 |
| FR-005 | 视频分辨率选择 | 中 | ✅ 已完成 | 100% | 前端表单和后端Schema已支持 |
| FR-006 | 批量导入视频链接 | 高 | ⚠️ 部分完成 | 40% | 批量创建API已实现，无批量下载 |
| FR-007 | 下载进度与状态显示 | 中 | ⚠️ 部分完成 | 30% | 状态字段已定义，无实时进度 |
...
| 视频下载 | yt-dlp | ⚠️ 部分完成 | 10% | 依赖已安装，未集成到业务逻辑 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 视频管理 | 5 | 5 | 0 | 0 | 100%（基础CRUD） |
| 处理任务 | 3 | 3 | 0 | 0 | 40%（仅状态切换，无实际处理） |
| 语料数据 | 3 | 0 | 3 | 0 | 10% |
| 标注管理 | 4 | 0 | 4 | 0 | 5% |
| **总计** | **15** | **8** | **7** | **0** | **28%** |
...
1. **实现视频下载核心逻辑**
   - 集成 yt-dlp 实现 YouTube/Bilibili/Vimeo 下载
   - 添加下载进度跟踪
   - 实现本地文件上传
