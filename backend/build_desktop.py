...
FFMPEG_SOURCE = Path(r"C:\Users\11628\AppData\Local\Microsoft\WinGet\Packages\yt-dlp.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-N-124716-g054dffd133-win64-gpl\bin")
...
if FFMPEG_SOURCE.exists():
    ffmpeg_exe = FFMPEG_SOURCE / "ffmpeg.exe"
    if ffmpeg_exe.exists():
        INCLUDE_BINARIES.append((str(ffmpeg_exe), "ffmpeg"))
...
def build():
    ...
    for src, dst in INCLUDE_BINARIES:
        sep = ";" if sys.platform == "win32" else ":"
        cmd.extend(["--add-binary", f"{src}{sep}{dst}"])

    cmd.append(str(BASE_DIR / "desktop_app.py"))
